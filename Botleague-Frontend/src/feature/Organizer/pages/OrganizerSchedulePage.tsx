...
function SmartSportPanel({
  ...
  if (loading) return <div className="py-8 text-center text-neutral-400">Loading…</div>;
  ...
  return (
    <div className="rounded-xl bg-white/3 p-8 text-center">
      <p className="text-neutral-400 text-sm mb-1">
        Sport is in <span className="text-white font-medium">{sport.status}</span> state.
      </p>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onReload} className="ml-4 shrink-0 rounded-lg bg-white/8 px-3 py-2 text-xs text-neutral-300 hover:bg-white/12 transition-colors">
          Refresh
        </button>
      </div>
