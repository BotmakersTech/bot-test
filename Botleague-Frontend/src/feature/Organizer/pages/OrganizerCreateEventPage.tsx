...
export default function OrganizerCreateEventPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const message = err?.response?.data?.message || err?.message || "Failed to create techfest"
      setError(message)
    } finally {
      setSubmitting(false)
      setUploadingImage(false)
    ...
    <div className="relative min-h-screen overflow-hidden bg-white">
      ...
      <div className="relative w-full p-8">
        ...
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500" style={{ fontFamily: "'Inter', sans-serif" }}>
              Starts as a draft — add sports and publish when ready
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          {/* ── 4. Event Dates ── */}
          <SectionBadge n={4} title="Techfest Dates" />
        ...
        <div className="mt-6 flex items-center justify-center pb-4">
          ...
          >
            ...
            {uploadingImage ? "Uploading image…" : submitting ? "Creating techfest…" : "Create Techfest"}
          </button>
        </div>
      </div>
    </div>
