...
export default function VolunteerCTA({ eventId, eventName, volunteersNeeded }: VolunteerCTAProps) {
  ...
  function renderMessage() {
    if (!isAuthenticated) {
      return "This techfest is looking for volunteers. Sign in with a Volunteer account to apply.";
    }
    if (!isVolunteerEligible) {
      return "This techfest is looking for volunteers. Only accounts registered as a Volunteer can apply.";
    }
    if (!loaded) return "Checking your application status…";
    if (!application) return "Help make this techfest happen — apply to volunteer today.";
