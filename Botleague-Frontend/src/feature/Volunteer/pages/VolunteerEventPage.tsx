...
// "Browse Events" page (UserEventPage.tsx), plus a "My Events" filter
...
import { useEvent } from "../../Event/hook/useEvent";
import type { EventResponse } from "../../Event/api/event.api";
import { getMyVolunteerAssignments } from "../../Event/api/volunteerApplication.api";
...
function EventCard({ event, mine, onClick }: { event: EventResponse; mine: boolean; onClick: () => void }) {
      ...
      <div className="flex items-start gap-4 flex-1 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {mine && (
              <span className="bg-[#EEF1FF] text-[#4F6EF7] text-[10px] font-bold rounded-full px-2 py-0.5 border border-[#DCE1FA] uppercase tracking-wide">
                My Event
              </span>
            )}
          </div>
...
export default function VolunteerEventPage() {
  ...
  useEffect(() => {
    getMyVolunteerAssignments()
      .then((assignments) => setMyEventIds(new Set(assignments.map((a) => a.eventId))))
      .catch(() => setMyEventIds(new Set()));
  }, []);
  ...
  const filtered = events.filter(
    (e) => matchesSearch(e) && (!status || e.status === status) && (!city || e.city === city) && (!mineOnly || myEventIds.has(e.id))
  );

  const suggestions = search ? events.filter(matchesSearch).slice(0, 6) : [];
  const activeFilters = [status, city, mineOnly ? "mine" : ""].filter(Boolean).length;

  const applyFilters = () => { setStatus(draftStatus); setCity(draftCity); setMineOnly(draftMineOnly); };
  const resetFilters = () => { setDraftStatus(""); setDraftCity(""); setDraftMineOnly(false); setStatus(""); setCity(""); setMineOnly(false); setSearch(""); };

  const goToEvent = (id: string) => navigate(`/events/${id}`);

  return (
    <div className="evt-page min-h-screen pt-11.5 px-13 pb-17.5 relative overflow-hidden">
          ...
          <div className="flex gap-4 flex-wrap items-end">
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              <label className="evt-input w-full h-[42px] px-3 flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-[13.5px] evt-font-poppins text-gray-700">My Events only</span>
              </label>
            </div>
          </div>
        </div>
        ...
        {!loading && !error && events.length > 0 && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center gap-2">
            ...
            <p className="evt-font-poppins text-[13px] text-gray-400">
              {mineOnly ? "You're not volunteering for any live events right now." : "Try adjusting your filters or search query."}
            </p>
          </div>
        )}
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="evt-fade-in flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              {filtered.map((ev) => (
                <EventCard key={ev.id} event={ev} mine={myEventIds.has(ev.id)} onClick={() => goToEvent(ev.id)} />
              ))}
            </div>
          </div>
        )}
